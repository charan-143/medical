...
document.addEventListener("DOMContentLoaded", function () {
			...
			fetch(`/dashboard/regenerate_summary/${folderId}`, {
				...
				.then((data) => {
					if (data.success) {
						...
						if (summaryTimestamp) {
							summaryTimestamp.textContent = `Last updated: ${formatTimestamp(
								data.last_updated
							)}`;
						}
	...
	// If we have summary text, render it with markdown
	if (summaryText && summaryText.innerHTML.trim()) {
		// Replace the content with rendered markdown
		summaryText.innerHTML = renderMarkdown(summaryText.innerHTML);
	}
});
